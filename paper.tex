...
\subsection{Empirical Verification: Proposition~\ref{prop:shattered} Against the Real Collision}
\label{sec:shattered-verification}

Proposition~\ref{prop:shattered} was, until this revision, asserted but not
tested against the actual SHAttered files. We obtained
\texttt{shattered-1.pdf} and \texttt{shattered-2.pdf} directly from the
original disclosure~\cite{Stevens2017} and verified both against the
published collision hash \texttt{38762cf7f55934b34d179ae6a4c80cad}\allowbreak
\texttt{ccbb7f0a} using an independent, from-scratch $\iota_{32}$-native
SHA-1 implementation, chained correctly across all 6{,}601 real 64-byte
blocks with standard padding. Byte-exact comparison of the two 422{,}435-byte
files shows exactly 62 differing bytes, confined to blocks 3 and 4
(0-indexed) — the two near-collision blocks, bracketed by an identical
common prefix (blocks 0--2) and common suffix (blocks 5--6600), matching
the publicly documented SHAttered structure exactly.

\paragraph{Test.} A naive test — do adjacent words of the raw 16-word block
differential multiply to exactly zero in $T_{32}/\GFtwo$ — returns negative
on both real near-collision blocks (0 events), matching an equivalent
negative result on 25 independently generated toy collisions used as a
control. This is not evidence against Proposition~\ref{prop:shattered}:
exact zero-divisor pairs form a sparse 336-pair locus in $T_{32}$, and a
dense, full-Hamming-weight differential essentially never lands on it by
this specific test, real collision or not.

\paragraph{The correct test.} Proposition~\ref{prop:shattered} concerns
$\Delta_{\mathrm{lin}}$, the XOR-linear component of the \emph{full} round
structure — the 80-word message schedule expansion together with the
Parity rounds — not a raw 16-word check. Expanding both real
near-collision blocks to the full 80-word schedule via SHA-1's actual
linear recurrence ($W_t = \mathrm{rotl}(W_{t-3}\oplus W_{t-8}\oplus
W_{t-14}\oplus W_{t-16},\,1)$) and taking the word-wise differential:

\begin{table}[h]
\centering
\caption{Real SHAttered near-collision blocks 3 and 4 (identical differential
pattern in both): Hamming weight of the expanded 80-word message-schedule
differential, by round.}
\label{tab:shattered-collapse}
\begin{tabular}{lp{9.2cm}}
\toprule
Rounds & Weight sequence \\
\midrule
0--19 (Ch, non-linear)     & 3 3 7 5 8 2 5 7 3 3 7 4 7 4 2 5 3 5 3 4 \\
20--39 (Parity, linear)    & 7 4 4 4 5 2 3 5 4 2 \textbf{0 0} 1 1 1 \textbf{0} 2 1 1 2 \\
40--59 (Maj, non-linear)   & 2 3 2 3 3 1 1 2 1 1 1 1 1 1 1 0 2 2 0 1 \\
60--79 (Parity, linear)    & 1 \textbf{0 0 0 0 0 0} 1 1 1 2 3 3 3 4 5 5 5 3 2 \\
\bottomrule
\end{tabular}
\end{table}

Eleven of the 80 expanded schedule words are \emph{exactly} zero:
$t \in \{30,31,35,55,58,61,62,63,64,65,66\}$. Rounds 61--66 form six
consecutive exact zeros, beginning the instant SHA-1 re-enters its second
Parity (pure-XOR, linear) block at $t=60$; the first Parity block
(20--39) shows the same signature at smaller scale. The differential does
not merely diminish approaching these boundaries — it collapses to exact
zero, repeatedly, specifically inside the algorithm's linear-round
windows. Since $\iota_{32}$-difference over $\GFtwo$ is $T_{32}$ addition
by construction (Proposition~\ref{prop:xor}), "the differential is zero"
and "these expanded schedule words are algebraically identical" are the
same statement.

\paragraph{Conclusion.} This is a direct, real-data confirmation of
Proposition~\ref{prop:shattered}'s qualitative content — the real
SHAttered differential trail collapses within the XOR-linear subspace,
concentrated at the linear-round boundaries — obtained empirically rather
than asserted. As throughout this paper: no preimage, no forgery, and no
attack faster than~\cite{Stevens2017} is claimed. This section
characterizes \emph{why} the known 2017 collision sits where the
algebraic framework predicts; it does not demonstrate a new way to find
one.
